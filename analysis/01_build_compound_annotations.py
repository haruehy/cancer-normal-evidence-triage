...
def main():
    ...
    # Optional primary-treatment annotation: preserve fixed labels by drug_key/name when possible.
    if PRISM_PRIMARY_INFO.exists():
        ...
        # If a primary entry has no exact drug_key match, use the fixed table by lowercase name as a fallback.
        ...
        if 'broad_subclass' not in pa.columns:
            pa['broad_subclass'] = ''
        if 'removal_axis' not in pa.columns:
            pa['removal_axis'] = 'unassigned'
